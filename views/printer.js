...
const PrinterView = {
      ...
      if (mode === 'by-assignment') {
        ...
        students.forEach(student => {
          for (let i = 0; i < copies; i++) {
            totalStickers++;
            const sticker = PrinterView.createSticker(student, assignment);
            page.appendChild(sticker);
            previewList.appendChild(PrinterView.cloneSticker(sticker));
          }
        });
  ...
  // sticker.cloneNode(true) だけだと<canvas>に描画済みの内容がコピーされない（属性のみクローンされる）ため、
  // 元のcanvasから新しいcanvasへピクセルをdrawImageで転写してから返す
  cloneSticker(sticker) {
    const clone = sticker.cloneNode(true);
    const originalCanvases = sticker.querySelectorAll('canvas');
    const clonedCanvases = clone.querySelectorAll('canvas');
    originalCanvases.forEach((orig, i) => {
      const target = clonedCanvases[i];
      if (target) {
        target.getContext('2d').drawImage(orig, 0, 0, target.width, target.height);
      }
    });
    return clone;
  },
  ...
  createSticker(student, assignment) {
    ...
    // TailwindのCDN(JIT)が任意バリアント([&>img]:w-full 等)を反映しない環境でも
    // QRコードが正しく表示されるよう、インラインstyleで直接サイズを固定する
    qrDiv.querySelectorAll('img, canvas').forEach(el => {
      el.style.width = '100%';
      el.style.height = '100%';
    });
