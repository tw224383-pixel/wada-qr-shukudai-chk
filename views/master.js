...
const MasterView = {
                  ...
                  ${data.assignments.map(a => `
                    <tr>
                      ...
                      <td class="px-3 py-2 text-right whitespace-nowrap">
                        <button class="text-blue-500 hover:text-blue-700 mr-2" onclick="renameAssignment('${a.id}')" title="名前を変更"><i data-lucide="pencil" class="w-4 h-4"></i></button>
  ...
  afterRender() {
    ...
    window.renameAssignment = (id) => {
      const current = store.data.assignments.find(a => a.id === id);
      if (!current) return;
      const newTitle = prompt('新しい提出物名を入力してください', current.title);
      ...
      store.updateAssignment(id, { title: trimmed });
      utils.showToast('提出物名を変更しました');
      window.app.navigate('master');
    };
